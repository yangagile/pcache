...
@Component("ops-AWS")
public class AwsS3Ops implements DataOps {

    @Value("${aws.client.s3.connectionTimeout:5000}")
    int connectionTimeout;
    @Value("${aws.client.s3.socketTimeout:3000}")
    int socketTimeout;
    @Value("${aws.client.s3.connectionMaxIdleTime:60000}")
    int connectionMaxIdleTime;
    @Value("${aws.client.s3.maxConnections:64}")
    int maxConnections;
    @Value("${aws.client.s3.tcpKeepAlive:true}")
    boolean tcpKeepAlive;
    @Value("${aws.role.policy:iam::243076431111:role/pc-fullRole}")
    String rolePolicy;
    ...
    public StsToken applyToken(ApplyStsToken applyStsToken, ClientCreationInfo clientCreationInfo) {
        ...
        String roleArn;
        if (IamUtils.isCNPartition(clientCreationInfo.getRegion())) {
            roleArn = "arn:aws-cn:" + rolePolicy;
        } else {
            roleArn = "arn:aws:" + rolePolicy;
        }
    ...
    public List<String> getPermissions(List<PcPermission> permissions) {
        ...
        for (PcPermission permission : permissions) {
            switch (permission) {
                case PutObject:
                    permissionList.addAll(Arrays.asList("PutObject", "AbortMultipartUpload",
                            "CompleteMultipartUpload", "CreateMultipartUpload",
                            "ListMultipartUploads", "ListParts", "UploadPart", "UploadPartCopy"));
                    continue;
                case GetObject:
